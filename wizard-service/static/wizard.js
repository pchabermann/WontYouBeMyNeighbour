...
function updateNetworkNameBadge() {
    ...
    if (networkNameBadge) {
        ...
        else if (wizardState && wizardState.deployment_config && wizardState.deployment_config.topology_name) {
            name = wizardState.deployment_config.topology_name;
        }

        if (name) {
            networkNameBadge.textContent = `Topology: ${name}`;
...
async function importNetworkTemplate() {
    ...
    try {
        ...
        if (networkData.docker || networkData.id) {
            wizardState.deployment_config = {
                topology_name: networkData.docker?.n || networkData.id,
                platform: 'kubernetes',
                namespace: `topology-${networkData.docker?.n || networkData.id}`
            };
        }
...
async function createSession() {
    try {
        ...
        const response = await fetch(`/api/wizard/session/${sessionId}/init`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({})
        });
        ...
        // Don't show alert - session can work without backend storage
        // Just use client-side session ID
        if (!sessionId) {
            sessionId = 'session-' + Date.now() + '-' + Math.random().toString(36).substr(2, 9);
        }
        console.log('Using client-side session ID:', sessionId);
    }
}
...
// Kubernetes Deployment - No longer needed for Docker network configuration
function updateDockerNetworkFields() {
    // No-op: Kubernetes handles networking automatically via CNI
    console.log(`[Wizard] Kubernetes deployment - CNI handles networking`);
}

// Kubernetes Platform Check (replaces Docker check)

async function checkDocker() {
    // No-op: Kubernetes platform is assumed available
    // The wizard service itself runs in Kubernetes
    console.log('[Wizard] Running on Kubernetes platform');
}
...
function saveMcpConfig() {
    ...
    const mcpName = currentMcpConfig.n;
    closeMcpModal();
    renderMcpGrid();
    ...
    showAlert(`${mcpName} configured and enabled!`, 'success');
}
...
async function nextStep(currentStep) {
    ...
    switch (currentStep) {
        case 1:
            ...
            if (!networkName) {
                showAlert('Please enter a topology name', 'error');
                return;
            }
            ...
            // Kubernetes deployment - no Docker-specific config needed
            wizardState.deployment_config = {
                topology_name: networkName,
                platform: 'kubernetes',
                namespace: `topology-${networkName}`,
...
async function saveStep1() {
    try {
        await fetch(`/api/wizard/session/${sessionId}/step/1`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(wizardState.deployment_config)
        });
    } catch (error) {
...
async function saveStep2() {
    try {
        await fetch(`/api/wizard/session/${sessionId}/step/2`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function saveStep4() {
    ...
    try {
        await fetch(`/api/wizard/session/${sessionId}/step/4`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function updatePreview() {
    try {
        ...
        await fetch(`/api/wizard/session/${sessionId}/step/5`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        document.getElementById('preview-agents').textContent = wizardState.agents.length;
        document.getElementById('preview-links').textContent = wizardState.topology?.links?.length || 0;
        document.getElementById('preview-mcps').textContent = wizardState.mcp_selection?.selected?.length || 0;
        document.getElementById('preview-pods').textContent = wizardState.agents.length;

        const topologyName = wizardState.deployment_config?.topology_name || 'my-network';
        document.getElementById('preview-details').innerHTML = `
            <p><strong>Topology:</strong> ${topologyName}</p>
            <p><strong>Namespace:</strong> topology-${topologyName}</p>
...
async function launchNetwork() {
    ...
    try {
        ...
        showDeployProgress();
        setDeployStatus('Deploying Topology...', 'Starting pods and configuring agents...');
        updateDeployStep('containers', 'active');

        showAlert('Launching topology... Please wait for pods to start.', 'info');
        console.log('Launching topology with session:', sessionId);

        const response = await fetch(`/api/wizard/session/${sessionId}/launch`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                topology_name: wizardState.deployment_config?.topology_name || 'my-network',
                api_keys: apiKeys
            })
        });
        ...
        // Pods launched successfully
        updateDeployStep('containers', 'complete');
        showAlert('Topology launched! Showing results...', 'success');
        ...
        // Build launch summary page - handle both Docker and K8s agent formats
        const allAgents = Object.entries(data.agents || {});
        console.log('All agents in response:', allAgents.length);
        const topologyNs = data.namespace || '';
        ...
        // Build agent display list - works for both Docker (webui_port) and K8s (namespace) agents
        const agentsWithWebUI = allAgents.map(([agentId, agent]) => {
            const agentConfig = wizardState.agents.find(a => a.id === agentId);
            ...
            return {
                id: agentId,
                port: agent.webui_port || null,
                ip: agent.ip_address || agent.dashboard_url || 'N/A',
                status: agent.status || 'deploying',
                namespace: agent.namespace || topologyNs,
                links: agent.links || 0,
                url: agent.webui_port
                    ? `http://localhost:${agent.webui_port}/dashboard?agent_id=${encodeURIComponent(agentId)}${hash}`
                    : '#'
            };
        });

        console.log('Agents for display:', agentsWithWebUI.length, agentsWithWebUI);
            ...
            // Determine if this is a K8s deployment (no webui_port) or Docker deployment
            const isK8s = agentsWithWebUI.length > 0 && !agentsWithWebUI[0].port;

            // Create launch summary HTML
            let summaryHTML = `
                <div style="text-align: center; margin-bottom: 30px;">
                    <h2 style="color: #4ade80; margin-bottom: 10px;">Network Deployed Successfully!</h2>
                    <p style="color: #888; margin-bottom: 20px;">${agentsWithWebUI.length} agent(s) deployed${isK8s ? ' to Kubernetes namespace: <code style="color: #00d9ff;">' + topologyNs + '</code>' : ''}</p>
            `;
            ...
            summaryHTML += `
                ...
                <div style="background: #1a1a2e; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
                    <h3 style="color: #00d9ff; margin-bottom: 15px;">${isK8s ? 'Deployed Agents' : 'Agent Dashboards'}</h3>
                    <div id="agent-status-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 15px;">
            `;

            for (const agent of agentsWithWebUI) {
                const statusColor = agent.status === 'deploying' ? '#facc15' : agent.status === 'running' ? '#4ade80' : '#ef4444';
                if (isK8s) {
                    const dashUrl = `/agent-proxy/${agent.namespace}/${agent.id}/dashboard?agent_id=${encodeURIComponent(agent.id)}`;
                    summaryHTML += `
                        <a href="${dashUrl}" target="_blank" style="text-decoration: none;">
                        <div data-agent-id="${agent.id}" style="background: #16213e; border: 2px solid ${statusColor}; border-radius: 8px; padding: 15px; transition: all 0.3s; cursor: pointer;"
                            ...
                            <div style="font-family: monospace; color: #4ade80; font-size: 0.9rem;">${agent.ip || 'N/A'}</div>
                            <div style="font-family: monospace; color: #888; font-size: 0.8rem;">${agent.links} link(s)</div>
                            <div class="agent-status" style="color: ${statusColor}; font-size: 0.75rem; margin-top: 8px; text-transform: uppercase;">${agent.status}</div>
                            <div style="margin-top: 10px;">
                                <span style="display: inline-block; padding: 6px 12px; background: #00d9ff; color: #1a1a2e; border-radius: 4px; font-size: 0.75rem; font-weight: 600;">Open Dashboard</span>
                            </div>
                        </div>
                        </a>
                    `;
            ...
            if (isK8s) {
                ...
                <div style="background: #1a1a2e; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
                    <h4 style="color: #00d9ff; margin-bottom: 10px;">kubectl Commands</h4>
                    <pre style="background: #0d1117; color: #c9d1d9; padding: 12px; border-radius: 6px; overflow-x: auto; font-size: 0.85rem;">kubectl get pods -n ${topologyNs}
kubectl logs -f deployment/${agentsWithWebUI[0]?.id || 'AGENT_NAME'} -n ${topologyNs}</pre>
                </div>`;
            } else {
                summaryHTML += `
                    </div>
            ...
            // Store topology name for refresh
            window.launchedTopologyName = data.topology_name;
            window.launchedNamespace = topologyNs;

            summaryHTML += `
                <div style="text-align: center; margin-bottom: 20px;">
                    <button onclick="refreshTopologyStatus()" id="refresh-pods-btn" style="background: #06b6d4; color: white; padding: 10px 24px; border: none; border-radius: 6px; cursor: pointer; font-size: 0.95rem; font-weight: bold;">
                        Refresh Pod Status
                    </button>
                    <span id="refresh-status" style="color: #888; margin-left: 10px; font-size: 0.85rem;"></span>
                </div>
            `;
            ...
            summaryHTML += `
                <div style="background: #1a1a2e; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
                    ...
                    <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                        <a href="http://localhost:8081/" target="_blank" style="background: #00d9ff; color: #1a1a2e; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold; display: inline-block;">
                            Full Agent Topology
                        </a>
                        <a href="http://localhost:8082/" target="_blank" style="background: #9333ea; color: white; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold; display: inline-block;">
                            3D Network View
                        </a>
                    </div>
                </div>
...
// Refresh topology pod status from K8s
async function refreshTopologyStatus() {
    const topologyName = window.launchedTopologyName;
    if (!topologyName) {
        console.warn('No topology name stored');
        return;
    }
    const btn = document.getElementById('refresh-pods-btn');
    const statusEl = document.getElementById('refresh-status');
    if (btn) btn.disabled = true;
    if (statusEl) statusEl.textContent = 'Fetching...';

    try {
        const response = await fetch(`/api/wizard/topologies/${topologyName}/pods`);
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();

        // Update the agent cards in the DOM
        ...
        for (const [agentId, info] of Object.entries(agents)) {
            ...
            if (card) {
                ...
                const ipEl = card.querySelector('.agent-ip');
                if (statusEl) {
                    statusEl.textContent = info.status.toUpperCase();
                    statusEl.style.color = info.status === 'running' ? '#4ade80' : '#facc15';
                }
                if (ipEl) ipEl.textContent = info.ip_address || 'N/A';
                card.style.borderColor = info.status === 'running' ? '#4ade80' : '#facc15';
            }
        }

        // If no data-agent-id cards exist, rebuild the agent grid
        if (agentCards.length === 0) {
            const grid = document.getElementById('agent-status-grid');
            if (grid) {
                grid.innerHTML = '';
                for (const [agentId, info] of Object.entries(agents)) {
                    const statusColor = info.status === 'running' ? '#4ade80' : '#facc15';
                    grid.innerHTML += `
                        <div data-agent-id="${agentId}" style="background: #16213e; border: 2px solid ${statusColor}; border-radius: 8px; padding: 15px;">
                            <div style="font-weight: bold; color: #00d9ff; margin-bottom: 5px;">${agentId}</div>
                            <div class="agent-ip" style="font-family: monospace; color: #4ade80; font-size: 0.9rem;">${info.ip_address || 'N/A'}</div>
                            <div class="agent-status" style="color: ${statusColor}; font-size: 0.75rem; margin-top: 8px; text-transform: uppercase;">${info.status}</div>
                        </div>
                    `;
                }
            }
        ...
        if (statusEl) statusEl.textContent = `${data.agent_count} pod(s) found - ${new Date().toLocaleTimeString()}`;
    } catch (err) {
