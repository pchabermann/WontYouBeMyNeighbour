...
class ASIKubernetesClient:
    ...
    def get_topology_graph(self, network_id: str) -> Dict[str, Any]:
        ...
        across all 3 layers: K8s pod network, ASI overlay, underlay.
        """
        ns = f"topology-{network_id}"
        try:
            pods = self.core.list_namespaced_pod(
        ...
        for pod in pods.items:
            ...
            # Build underlay_info from protocol config
            underlay_parts = []
            i = 0
            while True:
                ...
                if proto_type.lower() == "ospf":
                    underlay_parts.append(f"OSPF Area {area or '0'}")
                elif proto_type.lower() in ("bgp", "ibgp", "ebgp"):
                    asn = config_data.get(f"protocol.{i}.asn", "?")
                    underlay_parts.append(f"BGP AS{asn}")
                else:
                    underlay_parts.append(proto_type.upper())
                i += 1

            # Build overlay IPv6 from loopback (fd00:asi::{router_id_last_octet})
            router_id = config_data.get("protocol.0.routerId", "")
            overlay_ipv6 = None
            if router_id:
                # Generate a deterministic overlay IPv6 from the router-id
                parts = router_id.split(".")
                if len(parts) == 4:
                    overlay_ipv6 = f"fd00:a51:0:{parts[2]}::{parts[3]}/64"
            ...
            nodes.append({
                ...
                "underlay_info": ", ".join(underlay_parts) if underlay_parts else None,
                "port": None,
                "network": network_id,
            })
            agent_names.append(agent_name)

        # Layer 3: Underlay links from NADs (physical/Multus connections)
        links = self._build_links_from_nads(nads, pods)

        # Layer 1: K8s pod network links (all pods share the cluster network)
            ...
            for i in range(len(agent_names)):
                for j in range(i + 1, len(agent_names)):
                    links.append({
                        ...
                        "protocol": "K8s-Pod-Net",
                        "layer": "docker",
                        "name": f"k8s-{agent_names[i]}-{agent_names[j]}",
                    })

        # Layer 2: ASI overlay mesh (full mesh between all agents)
        if len(agent_names) >= 2:
            for i in range(len(agent_names)):
                for j in range(i + 1, len(agent_names)):
                    links.append({
                        ...
                        "layer": "overlay",
                        "name": f"overlay-{agent_names[i]}-{agent_names[j]}",
                    })
    ...
    def _build_frontend_config(self, config_data: Dict[str, str]) -> Dict[str, Any]:
        """
        Convert flat ConfigMap keys to the nested format topology3d JS expects.
        JS reads: config.n (name), config.protos ([{p: "ospf", ...}])
        """
        result: Dict[str, Any] = {
            "n": config_data.get("agent.name", ""),
        }
        # Build protos array from protocol.N.* keys
        protos = []
        i = 0
        while True:
            proto_type = config_data.get(f"protocol.{i}.type")
