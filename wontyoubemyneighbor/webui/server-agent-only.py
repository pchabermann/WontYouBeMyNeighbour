"""
ASI Agent Dashboard Server - Kubernetes Edition

FastAPI-based web dashboard for individual agents providing:
- Chat interface for ASI agentic assistant
- Real-time protocol status (OSPF/BGP neighbors, routes)
- Log streaming via WebSocket
- Console interface

This is the agent-only version WITHOUT the wizard UI.
The wizard is now a separate service that creates AgentTopology CRs.
"""
...
def create_webui_server(asi_app, agentic_bridge) -> FastAPI:
    ...
    async def startup_event():
        ...
        try:
            ...
            if asi_app and hasattr(asi_app, 'gre_tunnels') and asi_app.gre_tunnels:
                ...
                logging.getLogger("WebUI").info(f"GRE metrics enabled for agent {agent_id}")
            else:
                logging.getLogger("WebUI").debug(f"GRE metrics disabled for agent {agent_id} (no GRE interfaces)")
    ...
    # Otherwise create a new one for standalone mode
    ...
    async def root():
        """Serve the agent dashboard as landing page (K8s pod mode)"""
    ...
    async def dashboard():
        """Serve the full agent dashboard with protocol tabs (OSPF, BGP, Console, etc.)"""
        dashboard_file = static_dir / "agent-dashboard.html"
        if dashboard_file.exists():
            return FileResponse(str(dashboard_file))
